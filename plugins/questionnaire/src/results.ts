...
export default function (ctx: Context, _session: Session | null, opts: { query: string; by: string; total: number | null; results: Array<{ id: string; title: string; publisher?: string; status?: string }> }): string {
    ...
    // The whole row is the entity, not just the link inside it — that is what
    // puts the id/publisher cells within reach of whoever reads the row.
  ...
  ${r.publisher ? `<span class="w-40 shrink-0 truncate text-2xs text-text-tertiary" ${ctx.fns.ui.attr({ role: "publisher" })}>${esc(r.publisher)}</span>` : ""}
    ...
    return `<form id="qr-results" class="mt-4 overflow-hidden rounded-md border border-border-subtle" ${ctx.fns.ui.attr({ form: "qr-compare" })}
  ...
  <div class="flex items-center justify-between gap-3 bg-bg-tertiary px-4 py-2 text-2xs text-text-tertiary">
    ...
    <button class="text-text-link hover:underline" ${ctx.fns.ui.attr({ action: "compare" })}>Compare selected</button>
  </div>
