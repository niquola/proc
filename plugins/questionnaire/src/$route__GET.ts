...
export default async function (ctx: Context, _session: Session, opts: { req: Request }) {
  ...
  ${ctx.fns.ui.attr({ entity: "questionnaire", id: q.id, status: q.status })}>
  <span class="min-w-0 flex-1 truncate text-text-link" ${ctx.fns.ui.attr({ role: "title" })}>${esc(q.title)}</span>
  <span class="w-40 shrink-0 truncate font-mono text-2xs text-text-tertiary" ${ctx.fns.ui.attr({ role: "id" })}>${esc(q.id)}</span>
  <span class="shrink-0 text-2xs text-text-tertiary" ${ctx.fns.ui.attr({ role: "questions" })}>${q.items} questions</span>
</a>`;

    return {
        title: "questionnaires",
        main: `<section ${ctx.fns.ui.attr({ page: "questionnaires" })}>
<h1 class="text-lg font-semibold">Questionnaires</h1>
...
<form class="mt-6 flex items-center gap-2" ${ctx.fns.ui.attr({ form: "qr-search" })}
  ...
  <button class="rounded-md bg-brand px-3 py-1.5 text-ui text-text-inverse hover:bg-brand-hover" ${ctx.fns.ui.attr({ action: "search" })}>Search</button>
</form>
