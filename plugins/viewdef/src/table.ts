...
export default function (ctx: Context, _session: Session | null, opts: { columns: Array<{ name: string }>; rows: any[]; error: string | null; table: string }): string {
    if (opts.error) {
  ...
  <span class="font-mono">${esc(opts.table)}</span> is not readable yet — materialize it. <span class="opacity-70">${esc(opts.error)}</span>
    ...
    return `<div class="overflow-x-auto rounded-md border border-border-subtle" ${ctx.fns.ui.attr({ role: "rows" })}>
