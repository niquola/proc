...
export default async function (ctx: Context, _session: Session, _opts: { req: Request }) {
    ...
    return {
        ...
        main: ctx.fns.ui.page({
            ...
            main: charts.length
                ? drawn.map(c => `<div ${ctx.fns.ui.attr({ entity: "chart", id: c.id })}>${ctx.fns.ui.box({
                    class: "mt-4",
                    title: c.title,
                    right: `<a class="text-text-link hover:underline" href="/chart/view?id=${encodeURIComponent(c.id)}"
      hx-get="/chart/view?id=${encodeURIComponent(c.id)}" hx-target="#main" hx-swap="innerHTML" hx-push-url="true">open</a>`,
