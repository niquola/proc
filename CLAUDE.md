...
**By discovery (the workspace way).** A plugin is a directory with an `atomic-workspace.json` (`{ namespace?, src?, label?, icon?, description? }`) — and **a plugin is a skill directory**: the project keeps its own in `WORKDIR/.claude/skills/<name>/`, where the coding agent finds them by `SKILL.md` and the workspace by the manifest. Four sources, and the tier is what `workspace.json` can change: `core` (the workspace's own `plugins/`) and `project` (`WORKDIR/.claude/skills/*`) are always on; the global skill dirs (`~/.claude/skills`, `~/.agent/skills`, `~/.codex/skills`) are a **catalogue** mounted only when named in `workspace.json`; an `external` is a git repo cloned into `WORKDIR/.claude/skills/<name>`. `PLUGIN_PATHS` replaces the search list. What a plugin IS comes from its files: functions make it a library, a `GET /<namespace>` route makes it a tab, a `SKILL.md` makes it a skill, a `$hook_service.<x>.ts` makes it a service provider, and `"preview": { "files": "$qr_*.json", "fn": "preview" }` in the manifest makes it the viewer for those files in the file manager — `ctx.state.plugins` holds one record per mounted plugin (see `src/plugins/$state_plugins.ts`), read by the tab strip, the manager at `/plugins` and the agent's index alike. Managing: `plugins.list/catalog/add/remove/fetch/reload` — installing is a line in `workspace.json` plus a reload, never a restart. Full guide in `docs/plugins.md`.

**By declaration (npm/git).** A plugin is a package (local dir / npm / git) with a `proc` field in its `package.json` (`{ namespace, src }`) and a `src/` tree of normal proc functions. Declared in the **host** `package.json` `proc.plugins: [{ from }]` (`from` = a `bun add` spec). `project/roots.ts` resolves each plugin's dir and `project/scan.ts` prefixes its namespace onto each file's path before `classify` (keeping `abs` at the real file) — so plugin code merges into the **one shared `ctx.fns`** under its namespace (`auth/login.ts` → `ctx.fns.auth.login`, route → `GET /auth/...`) and flows through loadFns / genTypes / lint / loadRoutes / manifest+build unchanged. `genTypes` and `dev.manifest` import by path relative to `entry.abs`, so plugin files outside `src/` are typed and bundled into the single `dist/app.js`. `dev.lint` guards namespace collisions across plugins. Surface: `ctx.fns.plugins.add({from})` (bun add → persist → remount, dev-only), `.list({})`, `.remove({from})`. Broken plugins are logged + skipped at boot. Example: `plugins/filemanager`. Full guide in PLUGINS.md.
